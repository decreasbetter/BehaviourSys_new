...
if model_type.lower() == 'yolov5':
    ...
    from model.yoloV5.utils.general import non_max_suppression, scale_coords
...
class Yolov5(DetectMultiBackend):
    ...
    def postProcess(self, pre):
        ...
        pre = non_max_suppression(pre, conf_thres=self.conf_thres, iou_thres=self.iou_thres)[0]
        ...
        if len(pre):
            ...
            for obj in pre:
                cls = int(obj[5])  # 目标类别
                obj_info = objInfo(obj[:5].astype(int))  # 目标框

                meta_object[self.names[cls]].append(obj_info)
        return meta_object